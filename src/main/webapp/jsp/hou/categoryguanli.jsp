	...
	<div class='content'>
		<div class='neirong'>
			<div class="biao clearfix">
				<form action="CategorySelectAllServlet" method="get" id="sub" class="clearfix">
					...
					<!-- 隐藏yu -->
					<input name="flag" value='yes' type="hidden">
					<input name="pagesize" value='5' type="hidden">
					<input name="curpage" value="${param.curpage==null?1:param.curpage}" type="hidden">
				</form>
				<div class="context clearfix">
					<button class="btn">新增类别</button>
				</div>
				<div class='data'>
				<div id='fe'>
					<table class="table" border="1" cellspacing='0'>
						...
						<c:forEach items="${category }" var="item">
							<tr>
								...
								<td class="ntd">${item.state ==0?'不可用':'可用' }</td>
								<td class="ntd">
									<button value='${item.id}' class='an del'>删除</button>
									<button value='${item.id}' class='an shen'>审核</button>
								</td>
							</tr>
						</c:forEach>
					...
					<x:page controller='CategorySelectAllServlet' pagesize="5" total="${total}"
			curpage="${curpage}" where="${sql }" />
				</div>
